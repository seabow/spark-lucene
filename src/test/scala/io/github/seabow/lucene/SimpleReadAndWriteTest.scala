...
class SimpleReadAndWriteTest extends AnyFunSuite with Logging with SparkSessionTestWrapper with DatasetComparer with BeforeAndAfterAll{
  ...
  val expectedData_01 = List(
    ...
    Row(false,Date.valueOf("2023-05-30"),Timestamp.valueOf("2023-05-30 00:00:01") ,
      ...
      Map("color" -> "yellow", "0.3" -> "1","testadd"->"ok"),Map("sport"->Array("football", "basketball", "tennis")), Row("David Johnson", 40,Array("football", "basketball", "tennis")),20230530),
    Row(false,Date.valueOf("2023-05-31"),Timestamp.valueOf("2023-05-31 00:00:01") ,
      ...
      Map( "0.3" -> "1","color" -> "blue"), Map(),Row("Sarah Williams", 35,Array()),20230531),
  ...
  test("spark-sql tempview"){
    val df=spark.read.lucene("spark_lucene")
    df.createOrReplaceTempView("spark_lucene")
    spark.sql("select count(*) from spark_lucene group by MapInfo.`color`").explain(true)
  }
